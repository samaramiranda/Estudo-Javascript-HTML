...
(function () {
  ...
  */
  let newBrasil = []
  brasil.forEach(function (elem, ind) {
    newBrasil.push({ "id": ind, "estado": elem })
  })
  ...
  */
  ...
  let every = brasil.every(function (elem) {
  ...
  */
  ...
  let some = brasil.some(function (elem) {
    return elem === "Ceará"
  })
  ...
  */
  let map = newBrasil.map(function (elem) {
    return {
      id: elem.id + 1,
      estado: elem.estado += ` pertence ao Brasil`
    };
  })
  ...
  */
  let filter = map.filter(function(elem){
    return elem.id % 2 === 0
  })
